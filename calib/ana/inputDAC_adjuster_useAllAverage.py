...
"""
def shift_inputDAC(inputDAC_series: pd.Series, gain_series: pd.Series, gain_mean_all: float, gain_mean_cirasame: float) -> pd.Series:
    inputDAC_list = []
    for inputDAC, gain in zip(inputDAC_series.to_list(), gain_series.to_list()):
        inputDAC_list.append(int(inputDAC - GAIN_TO_INPUTDAC*(gain-gain_mean_all)))
...
def loop_process(workbook: dict, out: dict, i_cirasame: int, gain_mean_all: float) -> dict:
    ...
    inputDAC_series_shifted = shift_inputDAC(inputDAC_series, gain_series, gain_mean_all, gain_mean_cirasame)
